...
COMMENT ON COLUMN MEMBER.TEAM_STATUS IS '참여상태(Y:확정, N:임시 S:신청중)';
...
CREATE TABLE PROJECT(
    ...
    STATUS VARCHAR2(1) DEFAULT 'Y' CHECK(STATUS IN('Y','S','N'))
...
COMMENT ON COLUMN REPLY.REF_PRO IS 'PR 참조 게시물';   
COMMENT ON COLUMN REPLY.REF_PROJECTNO IS 'PROJECT 참조 게시물';
...
CREATE TABLE CALENDAR(
    ...
    BACKGROUND_COL VARCHAR2(100) NOT NULL,
    STATUS VARCHAR2(1) DEFAULT 'Y' CHECK (STATUS IN ('Y', 'N'))
);
...
COMMENT ON COLUMN CALENDAR.BACKGROUND_COL IS '일정 배경';
...
CREATE TABLE PULL_REQUEST(
    ...
    STATUS VARCHAR2(1) DEFAULT 'O' CHECK(STATUS IN ('O','C')) NOT NULL,
    CREATE_DATE DATE  DEFAULT SYSDATE NOT NULL
);
...
COMMENT ON COLUMN PULL_REQUEST.STATUS IS '상태(O:open, C:close)';
...
CREATE TABLE ALARMLIST(
    ...
    ALARM_TITLE VARCHAR2(1000) NOT NULL,
    ALARM_TYPE VARCHAR2(50) NOT NULL,
    MEM_ID VARCHAR2(50) REFERENCES MEMBER NOT NULL,
    RMEM_ID VARCHAR2(50) REFERENCES MEMBER NOT NULL,
    ALARM_CONTENT_NO NUMBER NOT NULL,
    ALARM_DATE DATE DEFAULT SYSDATE,
    ALARM_YN CHAR(1) DEFAULT 1 CHECK(ALARM_YN IN(0,1)),
    STATUS CHAR(1) DEFAULT 'Y' CHECK(STATUS IN('Y','N'))
);

COMMENT ON COLUMN ALARMLIST.ALARM_NO IS '알람번호';
COMMENT ON COLUMN ALARMLIST.ALARM_TITLE IS '알람제목';
COMMENT ON COLUMN ALARMLIST.ALARM_TYPE IS '알람타입';
COMMENT ON COLUMN ALARMLIST.MEM_ID IS '작성회원';
COMMENT ON COLUMN ALARMLIST.RMEM_ID IS '알람받을회원';
COMMENT ON COLUMN ALARMLIST.ALARM_YN IS '알람확인여부';
